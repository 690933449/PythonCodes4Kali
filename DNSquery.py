#!/usr/bin/env python
# DNS query program - C4 - DNSquery.py

import sys, socket, DNS, DNSany, re

def getreverse(query):
	"""Given the query, returns an appropriate reverse lookup string under IN-ADDR.ARPA if query is an IP address; otherwise, returns None. This function is not IBv6-cpmpatible."""
...
while len(queries):
	...
	for answer in answers:
		...
		if answer['typename'] == 'MX':
			print formatline(count, answer['typename'], 'Mail server', "%s priority %d" % (answer['data'][1], answer['data'][0]))
			queries.append((answer['data'][1], DNS.Type.A))
		elif answer['typename'] == 'SOA':
			data = "\n" + "\n".join([str(x) for x in answer['data']])
			print formatline(count, 'SOA', 'Start of anthority', data)
