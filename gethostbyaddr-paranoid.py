#!/usr/bin/env python
# Error-checking gethostbyaddr() exaple - C4 - gethostbyaddr-paranoid.py
# Performs a reverse lookup on the IP address given on the command line and sanity-checks the result
...
try:
	...
	print 'Got hostname %s, but it could not be forward-resolved: %s' % (host, str(e))
	sys.exit(0)

# If the forward lookup did not yield the original IP address anywhere, someonen is playing tricks. Explain the situation and exit.
